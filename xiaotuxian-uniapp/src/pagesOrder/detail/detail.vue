<template>
  ...
  >
    <template v-if="true">
      ...
      >
        <!-- 订单物流信息 -->
        <view class="item" v-for="item in shipment?.list" :key="item.id">
        ...
        <view class="locate">
          ...
          <veiw class="address">{{ order?.receiverAddress }}</veiw>
        </view>
      </view>
...
// 监听订单状态，获取订单物流信息
const shipment = ref<OrderLogisticResult>()
watch(
  () => order.value?.orderState,
  async (newValue) => {
    // 仅在订单状态为待收货，待评价，已完成时，可获取物流信息
    if ([OrderState.DaiShouHuo, OrderState.DaiPingJia, OrderState.YiWanCheng].includes(newValue!)) {
      const res = await getMemberOrderLogisticsByIdAPI(query.id)
      shipment.value = res.result
    } else {
      shipment.value = {} as OrderLogisticResult
    }
  },
)
</script>
